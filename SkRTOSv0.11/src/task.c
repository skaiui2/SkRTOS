...
void AbsoluteTimeAdd( void )
{
    ...
    atomic_inc( &(AbsoluteTime.one_levelTime) );
    if( (AbsoluteTime.one_levelTime) == 0)
    {
        atomic_inc( &(AbsoluteTime.two_levelTime) );
    }
}
...
void ReadylistInit( void )
{
    ...
    while( i != 0)
    {
        listInit(&(Readylist.Readylists[i]));
        i--;
    }
...
void ALLlistInit( void )
{
    ...
    listInit( &Suspendlist.xlist );
    listInit( &Blocklist.xlist );
...
void ReadyListAdd( TCB_struct *self )
{
    ...
    list_add( &(Readylist.Readylists[self->priority]), &(self->tasknode) );
    Readylist.Readylists[self->priority].SwitchFlag = Readylist.Readylists[self->priority].head->value;
}


void ReadyListRemove( TCB_struct *self )
{
    list_remove( &(Readylist.Readylists[self->priority]), &(self->tasknode) );

    if( (Readylist.Readylists[self->priority].count ) == 0)
    {
...
{
    self->IPC_node.value = self->priority;
    list_add( IPC_list , &(self->IPC_node));
}

void Remove_IPC(TCB_struct *self)
{
    list_remove( (thelist *)self->IPC_node.thelist , &(self->IPC_node));
    self->IPC_node.value = 0;
    self->IPC_node.thelist = NULL;
}


void SuspendlistAdd( TCB_struct *self)
{
    list_add( &(Suspendlist.xlist), &(self->tasknode) );
}

void SuspendlistRemove( TCB_struct *self)
{
    list_remove( &(Suspendlist.xlist), &(self->tasknode) );

}


void BlocklistAdd( TCB_struct *self)
{
    list_add( &(Blocklist.xlist), &(self->tasknode) );
}


void BlocklistRemove( TCB_struct *self)
{
    list_remove( &(Blocklist.xlist), &(self->tasknode) );
}
...
uint32_t this_cnt;//!!!Reserved, to be implemented functionality
uint32_t last_cnt;//!!!Reserved, to be implemented functionality
uint8_t cpu_use_rate()
{
    uint8_t amount_rate = 0;
    uint32_t difference = this_cnt - last_cnt;
    amount_rate += AbsoluteTime.one_levelTime*3000 & ((difference &~ 0x01) - 1);
    return amount_rate;
}
                    ...
                    )
{
    ...
    *pStack = (uint32_t)topStack;
    ...
    pxreturn = Stack_set(topStack,self,function_t);
...
volatile uint8_t PendSVFlag = 0;
__attribute__((always_inline))   void  vTaskSwitchContext( void )
{
    uint8_t Index= FindHighestPriority(); //debug by 'print Index'
    thelist *TopPrioritiesList = &( Readylist.Readylists[Index] );
    PendSVFlag++;
